...
class SummonerStats:
    ...
    async def mastery(self, ctx, *args):
        ...
        if args[-1].upper() in VALID_REGIONS:
            region = args[-1].upper()
            if args[-2].title() in open(r"C:\Users\Alex Palmer\OneDrive\Bots\BlitzcrankBotV2\spaces.txt").read():
            ...
            region = None
            if args[-1].title() in open(r"C:\Users\Alex Palmer\OneDrive\Bots\BlitzcrankBotV2\spaces.txt").read():
