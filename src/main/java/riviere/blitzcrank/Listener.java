        ...
        if (event instanceof ReadyEvent) {
            ...
            if (guild.getMembers().size() - botCount < 5 || (botCount > 20) && ((double) botCount / guild.getMembers().size() > 0.65)) {
                ...
                builder.setColor(0x1AFFA7)
                        ...
                        .addField("Server:", guild.getName(), true)
                        .addField("Users:", "" + guild.getMembers().size(), true)
                        .addField("Total:", "" + blitzcrank.getShardManager().getShardById(event.getJDA().getShardInfo().getShardId()).getGuilds().size(), true);
                TextChannel log = blitzcrank.getShardManager().getTextChannelById("295831639219634177");
                log.sendMessage(builder.build()).queue();
            }

        } else if (event instanceof MessageReceivedEvent) {
            if (((MessageReceivedEvent) event).getMessage().getContentRaw().startsWith("b!") && !((MessageReceivedEvent) event).isFromType(ChannelType.PRIVATE)) {
