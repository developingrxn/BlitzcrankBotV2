        ...
        event.async(() -> {
            ...
            try {
                ...
                try {
                    ...
                    if (hasPerms)
                        msg.editMessage(new EmbedBuilder().setColor(0xCA0147).setTitle("Error!").setDescription(String.format("%s is not in an active game!", summoner.getName())).build()).queue();
                ...
                if (hasPerms) {
                    ...
                    for (Player player : blueTeam.getParticipants()) {
                        blueField += String.format("**%s**\n%s\n\n", player.getSummoner().getName(), player.getChampion().getName());
                    }
                    for (Player player : redTeam.getParticipants()) {
                        redField += String.format("**%s**\n%s\n\n", player.getSummoner().getName(), player.getChampion().getName());
                    }
